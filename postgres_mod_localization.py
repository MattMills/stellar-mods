...
from parse_pdxlocalization import parser,check_path_pdxlocalization,parse_zip_file
...
cur.execute(
    ...
    #"  and 
    "lower(mods_filelist.filename) like '%.yml'"
    #"and mods.uuid = '8367c824-80a2-11ed-a7d9-8585a1519e40'"
    )
mod_files = {}
...
for mod_uuid in mod_files.keys():
    error = False
    if os.path.exists('%s/%s/.localization_v1_ingest_complete' % (mods_finished, mod_uuid)):
    ...
    for f in dirs:
        ...
        try: 
            for section in parse_zip_file(stats, zip_filename, True, False):
                ...
                cur.execute(cur.mogrify('insert into localizations_v1 (file_uuid, fileorder, lineno, key, number, value) values (%(file_uuid)s, %(order)s, %(lineno)s, %(key)s, %(number)s, %(value)s) on conflict do nothing', section))
            ...
            dbh.commit()
    if error == False:
        with open('%s/%s/.localization_v1_ingest_complete' % (mods_finished, mod_uuid), 'w') as fh:
